...
public class SysUserDAO extends SpringJDBCTemplate {
	...
	public int saveUser(SysUser sysUser) throws Exception {
		...
		final Object[] objs = {sysUser.getLoginName(),sysUser.getPassword(),sysUser.getUserName(),sysUser.getUserSex()
				,sysUser.getUserAge(),sysUser.getUserType(),sysUser.getHold()};
	...
	public void editUser(SysUser sysUser) throws Exception {
		String sql = "update sys_user set login_name=?,password=?,user_name=?,user_sex=?,user_age=?,user_type=?  where user_id=?";
		Object[] objs = {sysUser.getLoginName(),sysUser.getPassword(),sysUser.getUserName(),sysUser.getUserSex(),sysUser.getUserAge()
